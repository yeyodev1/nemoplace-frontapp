...
const filteredAccounts = computed(() => {
  if (!searchQuery.value) return props.availableAdAccounts;
  const q = normalizeText(searchQuery.value);
  return props.availableAdAccounts.filter((account: any) => 
    normalizeText(account.name).includes(q) || account.account_id.includes(q)
  );
});
</script>
    ...
    <div v-if="isOpen" class="modal-overlay">
      <div class="modal-content">
        <div class="modal-header">
          <h3>Selecciona la Cuenta Publicitaria</h3>
        </div>
